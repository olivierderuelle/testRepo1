	...
	stages {
		...
		stage('Prod Deploymentsss') {
			steps{
				...
				script {
					ecsTaskNewRevisionJsonResponse = sh (script: sh "aws ecs register-task-definition --cli-input-json file://${workspace}/awsTaskDefinitionTest1.json | egrep \"revision\" | awk '{print \$2}'",returnStdout: true).trim()
				}
